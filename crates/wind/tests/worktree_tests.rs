...
fn test_worktree_detection() -> Result<()> {
    ...
    let is_main_worktree = wind::worktree::is_worktree(repo_path)?;
    assert!(
        !is_main_worktree,
        "Main repo should not be detected as worktree"
    );

    let is_worktree = wind::worktree::is_worktree(&worktree_dir)?;
    assert!(is_worktree, "Should detect worktree");

    let gitdir = wind::worktree::get_gitdir(&worktree_dir)?;
...
fn test_list_worktrees() -> Result<()> {
    ...
    let worktrees = wind::worktree::list_worktrees(repo_path)?;
    assert_eq!(
        worktrees.len(),
        3,
...
fn test_branch_checked_out_protection() -> Result<()> {
    ...
    let is_checked_out = wind::worktree::is_branch_checked_out(repo_path, "feature")?;
    assert!(
        is_checked_out,
        "Feature branch should be checked out in worktree"
    );

    let is_main_checked_out = wind::worktree::is_branch_checked_out(repo_path, "main")?;
    assert!(
        is_main_checked_out,
...
fn test_worktree_operations() -> Result<()> {
    ...
    let repo = wind::Repository::init(repo_path)?;
    ...
    let repo2 = wind::Repository::open(&worktree_dir)?;
