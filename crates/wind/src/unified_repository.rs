...
";
        ...
        let working_copy = WorkingCopy::new(
            path.clone(),
            &wind_dir,
            storage.clone() as Arc<dyn wind_storage::SyncObjectStore>,
        )?;
        let merge_engine =
            MergeEngine::new(storage.clone() as Arc<dyn wind_storage::SyncObjectStore>);
    ...
    pub fn commit(&mut self, message: &str) -> Result<String> {
        ...
        for entry in index_entries {
            let file_change = ModelFileChange::Added {
                oid: entry.oid.clone(),
            };
