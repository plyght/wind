...
use wind_core::{FileStatus, UnifiedRepository};
...
fn test_status_with_renames() -> Result<()> {
    ...
    let has_rename = status.iter().any(|s| matches!(s, FileStatus::Renamed { .. }));
...
fn test_modified_files() -> Result<()> {
    ...
    let has_modified = status
        .iter()
        .any(|s| matches!(s, FileStatus::Modified) || matches!(s, FileStatus::Untracked));
    assert!(has_modified, "Should detect modified file, got: {:?}", status);
...
fn test_deleted_files() -> Result<()> {
    ...
    let has_deleted = status.iter().any(|s| matches!(s, FileStatus::Deleted));
...
fn test_untracked_files() -> Result<()> {
    ...
    let has_untracked = status.iter().any(|s| matches!(s, FileStatus::Untracked));
