...
fn test_submodule_detection() -> Result<()> {
    ...
    let has_subs = wind_core::submodule::has_submodules(&main_repo)?;
    assert!(has_subs, "Should detect submodules");

    let submodules = wind_core::submodule::list_submodules(&main_repo)?;
...
"#;
    ...
    let submodules = wind_core::submodule::list_submodules(repo_path)?;
...
"#;
    ...
    let sub = &submodules[0];
    let status = wind_core::submodule::get_submodule_status(repo_path, sub)?;
...
fn test_inside_submodule_detection() -> Result<()> {
    ...
    let is_inside = wind_core::submodule::is_inside_submodule(&submodule_path)?;
    assert!(is_inside, "Should detect being inside submodule");

    let is_root_inside = wind_core::submodule::is_inside_submodule(repo_path)?;
    assert!(
        !is_root_inside,
...
fn test_repository_submodule_integration() -> Result<()> {
    ...
    let repo = wind_core::Repository::init(&main_repo)?;
