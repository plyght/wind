...
fn test_rename_detection() -> Result<()> {
    ...
    fs::rename(repo_path.join("old_name.txt"), repo_path.join("new_name.txt"))?;
...
fn test_merge() -> Result<()> {
    ...
    match merge_result {
        wind_core::MergeResult::Clean { new_changeset_id } => {
            assert!(!new_changeset_id.is_empty());
        }
        wind_core::MergeResult::Conflicts { conflicts } => {
